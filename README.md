...
2. Ожидается завершение всех активных HTTP запросов (таймаут 30 сек)
3. Актуальное состояние in-memory хранилища (включая мягкие удаления) сбрасывается в JSON файл
4. Закрывается соединение с БД
5. Приложение корректно завершается

**Сигналы завершения:** `SIGINT` (Ctrl+C), `SIGTERM`, `SIGQUIT`.
